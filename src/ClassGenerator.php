...
abstract class ClassGenerator
{
     ...
     * @return int
     */
    public function runFull(string $swaggerFile, string $dir): int
    {
        $this->generate($swaggerFile);
    ...
    protected function saveClassesInternal(string $dir, string $namespace_name, string $use = ''): void
    {
        ...
        foreach ($this->classes as $class_name => $class) {
            $php_file = $printer->printClass($class);
    ...
    protected function typeFromRef(array $property): string
    {
        ...
        $ref = str_replace('#/definitions/', '', $property['$ref']);
        if (isset($this->typeMap[$ref])) {
            $ref = $this->typeMap[$ref];
        }
        return $ref;
    }
